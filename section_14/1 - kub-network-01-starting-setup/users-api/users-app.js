...
app.post('/signup', async (req, res) => {
  ...
  try {
    const hashedPW = await axios.get(`http://${process.env.SERV_AUTH_SERVICE_HOST}/hashed-password/` + password);
...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get(
    `http://${process.env.SERV_AUTH_SERVICE_HOST}/token/` + hashedPassword + '/' + password
  );
